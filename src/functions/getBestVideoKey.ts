...
type VideoTemp = {
    id: number;
    results: VideoResponseType[];
};

const videoTemp: VideoTemp = {
    "id": 1022789,
    "results": [
        {
            "iso_639_1": "en",
            ...
            "type": "Clip",
            "official": true,
            "published_at": "2024-08-19T16:10:39.000Z",
            "id": "66c437523af2077763c87e43"
        },
        {
            "iso_639_1": "en",
            "iso_3166_1": "US",
...
function getBestVideoKey(videoTempParameter: VideoResponseType[]): string | null {
    const filteredList = videoTempParameter.filter(c =>
        c.type === "Trailer" &&
        c.site === "YouTube" &&
        c.official === true
    );

    const bestVideo = filteredList.find(c => c.name === 'Final Trailer') || filteredList.find(c => c.name === 'Official Trailer') || null;

    return bestVideo ? bestVideo.key : (filteredList[0]?.key || null);
}

console.log(getBestVideoKey(videoTemp.results));